...
//Delete contact
	...
	//select Last and first to ask msg		
		if (isset($_POST['delete']) && isset($_POST['ask'])) {	
			$id=$_POST['id'];
			$sql = "DELETE contacts, best_phone FROM contacts JOIN best_phone ON id = '$id' AND id_contacts = '$id'"; 
	...
	$sql1 = "SELECT id FROM contacts  WHERE users_id = '$username' ";
	...
	// write contact records
		...
		if (isset($_GET["sort_l"]) or $sort_l =='last_ascending') {
			...
			if ($sort_l == 'last_ascending' ){
				$sql =  "SELECT id, last_name, first_name, cell_phone, work_phone, home_phone, email, best_phone FROM  contacts, best_phone WHERE best_phone.id_contacts = contacts.id AND contacts.users_id = '$username' ORDER BY last_name LIMIT $start_from, $num_records_per_page";
				$symbol_l= '&#9660';
				$sort_l = 'last_descending';
			}
			else{
				$sql = "SELECT id, last_name, first_name, cell_phone, work_phone, home_phone, email, best_phone FROM  contacts, best_phone WHERE best_phone.id_contacts = contacts.id AND contacts.users_id = '$username' ORDER BY last_name DESC LIMIT $start_from, $num_records_per_page";
				$symbol_l= '&#9650';
				$sort_l = 'last_ascending';	
			}
		}	
		if (isset($_GET["sort_f"])) {
			...
			if ($sort_f == 'first_ascending'){
				$sql = "SELECT id, last_name, first_name, cell_phone, work_phone, home_phone, email, best_phone FROM  contacts, best_phone WHERE best_phone.id_contacts = contacts.id AND contacts.users_id = '$username' ORDER BY first_name LIMIT $start_from, $num_records_per_page";
				$symbol_f= "&#9660";
				$sort_f = 'first_descending';
			}
			else{
				$sql = "SELECT id, last_name, first_name, cell_phone, work_phone, home_phone, email, best_phone FROM  contacts, best_phone WHERE best_phone.id_contacts = contacts.id AND contacts.users_id = '$username' ORDER BY first_name DESC LIMIT $start_from, $num_records_per_page";
				$symbol_f= '&#9650';
				$sort_f = 'first_ascending';
			}
		}
